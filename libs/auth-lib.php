...
{
    global $pdo;
    $hash = bin2hex(random_bytes(8));//random bytes in binery to hexadesimal
    //هش برای استفاده از نوکن
    $token = rand(100000, 999999);
    $expired_at = time() + 600;//10 minutes
...
{
    changeLoginSession($email);//پاک کردن سشن چون سشن میتواند از کوکی سرقت شود
    setcookie('auth', '', time() - 60, '/');//حذف کوکی
    redirect('auth.php');
}
